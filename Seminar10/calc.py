...
def start(update, _):
    update.message.reply_text(
        'Здравствуйте! Выберите с какими числами хотите сделать операцию.\n'
...
def choice(update, context):
    ...
    logger.info('Выбор оперции: %s: %s', user.first_name, update.message.text)
...
def operation_rational(update, context):
    user = update.message.from_user
    logger.info(
    ...
    if user_choice in '+-*/':
        ...
        update.message.rely_text('Введите +-*/')
...
def complex_two(update, context):
    ...
    if ' ' in test and (test.replace(' ', '')).isdigit():
        ...
        update.message.reply_text('Введите Re и Im второго числа через ПРОБЕЛ')
...
conversation_handler = ConversationHandler (
    ...
    states = {
        ...
        OPERATIONS_RATIONAL: [MessageHandler(Filters.text, operation_rational)],
